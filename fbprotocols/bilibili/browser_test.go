...
func TestLoginGateOpensBrowserOnce(t *testing.T) {
	savedStart, restore := startLoginUIFn, restoreBrowserSeams()
	defer func() {
		startLoginUIFn = savedStart
		restore()
	}()

	startLoginUIFn = func() (string, error) { return "http://127.0.0.1:1/", nil }
	isDesktopFn = func() bool { return true }
	n := 0
	openUrlFunc = func(string) error { n++; return nil }
	...
	for i := 0; i < 2; i++ {
		if err := loginGate(); err == nil {
			t.Fatalf("loginGate returned nil error on iter %d", i)
		}
	}
	if n != 1 {
		t.Fatalf("loginGate opened the browser %d times, want exactly 1", n)
	}
}
